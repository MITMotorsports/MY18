                                    ...
                                    VCU_ImplausibilityConflict *implaus_conflict)
{
  ...
  const uint16_t accel =
    (brake_throttle.accel_1 >
     brake_throttle.accel_2) ? brake_throttle.accel_2 : brake_throttle.accel_1;
  ...
  if (curr_conflict) {
    ...
    implaus_conflict->has_brake_throttle_conflict =  accel >=
                                                    CONFLICT_END_THROTTLE_TRAVEL;
  }
  ...
  implaus_conflict->has_brake_throttle_conflict = brake_engaged &&
                                                  throttle_engaged;
}
...
void update_implausibility(VCU_BrakeAndThrottle        brake_throttle,
  ...
  uint16_t max_travel =
    (brake_throttle.accel_1 >
     brake_throttle.accel_2) ? brake_throttle.accel_1 : brake_throttle.accel_2;
  uint16_t min_travel =
    (brake_throttle.accel_1 >
     brake_throttle.accel_2) ? brake_throttle.accel_2 : brake_throttle.accel_1;
